...
The model drafts an answer, a status, and citations. Code then:
  ...
  2. verifies every quote is verbatim in the cited file (one retry to fix bad quotes),
  3. downgrades an 'answered' response with no surviving citation to insufficient_evidence,
  4. drops any citation to a document customers may not see.
"""
...
from .config import CHAT_MODEL, EMBED_MODEL, MAX_QUOTE_CHARS, TOP_K, load_env, price_usd
from .policy import STALE_BEFORE, parse_date, plan_gate
...
5. Write for a customer: plain prose, no internal jargon, no file paths in the answer text,
   two to five sentences. Do not mention "the context" or "chunks".
...
def _format_context(hits: List[Tuple[Document, float]]) -> str:
    ...
    for i, (doc, _score) in enumerate(ordered, 1):
        ...
        warn = []
        updated = parse_date(m.get("last_updated", ""))
        if updated and updated < STALE_BEFORE:
            warn.append(f"STALE: last updated {updated.isoformat()}, may be out of date")
        if tier >= 4:
...
def quote_in_file(index: Index, path: str, quote: str) -> bool:
    ...
    if len(q) < 8:
        return False
    for source in (index.raw_text(path), index.extracted(path)):
        if source and q in normalize(source):
            return True
    return False
...
_STOP = {"the", "a", "an", "and", "or", "of", "to", "in", "on", "for", "with", "is", "are", "be", "it",
         "this", "that", "your", "you", "we", "our", "at", "by", "as", "from", "can", "will", "not", "if"}
...
def fallback_citation(answer: str, hits: List[Tuple[Document, float]], min_overlap: float = 0.5) -> Optional[Citation]:
    """Pick the retrieved sentence that best overlaps the answer. Used only when the model's own
    quotes failed verification twice; the quote is verbatim by construction."""
    ...
    for doc, _ in hits:
        for sent in _SENT.split(doc.page_content):
            ...
            if score > best_score:
                best, best_score = (doc.metadata["path"], sent[:MAX_QUOTE_CHARS]), score
...
class Answerer:
    ...
    def _call(self, messages) -> Tuple[Draft, int, int]:
        out = self.llm.invoke(messages)
        raw = out["raw"]
        usage = getattr(raw, "usage_metadata", None) or {}
        draft: Optional[Draft] = out.get("parsed")
        if draft is None:
            draft = Draft(status="insufficient_evidence", answer="I could not produce a reliable answer from the documentation. Please contact support@ferrowave.example.", citations=[], confidence=0.0, reason=f"parse error: {out.get('parsing_error')}")
        return draft, int(usage.get("input_tokens", 0)), int(usage.get("output_tokens", 0))
    ...
    def ask(self, question: str, k: int = TOP_K) -> Dict:
        ...
        if gate == "force":
            hint = ("\n\nNOTE: the customer has not said which plan they are on and this topic varies by plan. "
                    "Set status to needs_clarification and ask which plan (Starter, Growth, Scale, or Enterprise) "
                    "they are on. You may add one sentence on how the answer varies.")
        messages = [SystemMessage(content=SYSTEM_PROMPT),
                    HumanMessage(content=f"Customer question: {question}{hint}\n\nContext chunks:\n\n{context}")]
        draft, tin, tout = self._call(messages)
        tokens_in, tokens_out = tin, tout
        calls = 1

        visible = self.index.visible_paths()
        good, bad = self._verify(draft.citations, visible)
        if bad and draft.status != "insufficient_evidence":
            # One repair pass: ask for verbatim quotes for the ones that failed.
            repair = ("\n\nThe following citations were rejected because the quote is not verbatim in the file "
                      "(or the path is wrong). Re-issue them with exact copied text, or drop them:\n"
                      + "\n".join(f"- {c.path}: {c.quote!r}" for c in bad))
            messages.append(HumanMessage(content=repair))
            draft2, tin2, tout2 = self._call(messages)
            tokens_in += tin2
            tokens_out += tout2
            calls += 1
            good2, bad2 = self._verify(draft2.citations, visible)
            if len(good2) >= len(good):
                draft, good, bad = draft2, good2, bad2

        status: str = draft.status
        answer = draft.answer.strip()
        notes: List[str] = []
        if gate == "force" and status != "needs_clarification":
            status = "needs_clarification"
            answer = ("To answer that I need to know which plan your workspace is on (Starter, Growth, Scale, or "
                      "Enterprise). Which one is it? " + answer)
        ...
        if status == "answered" and not good:
            ...
            if fb is not None and fb.path in visible and quote_in_file(self.index, fb.path, fb.quote):
                good = [fb]
                notes.append("model quotes failed verification; citation selected by code from a retrieved chunk")
        if status == "answered" and not good:
            status = "insufficient_evidence"
            answer = ("I could not find a documented answer to that in the Ferrowave documentation I have access to. "
        ...
        return {
            ...
            "diagnostics": {
                ...
                "llm_calls": calls,
                "retrieved": [{"path": d.metadata["path"], "section": d.metadata.get("section", ""), "tier": d.metadata.get("tier"), "score": round(s, 4)} for d, s in hits],
                "plan_gate": gate,
    ...
    def _verify(self, citations: List[Citation], visible: set) -> Tuple[List[Citation], List[Citation]]:
        return verify_citations(self.index, citations, visible)


def verify_citations(index: Index, citations: List[Citation], visible: Optional[set] = None) -> Tuple[List[Citation], List[Citation]]:
    ...
    for c in citations:
        ...
        if key in seen:
            continue
        seen.add(key)
        if path in visible and quote_in_file(index, path, c.quote):
            good.append(c)
        else:
            bad.append(c)
    return good, bad
